...
### 1.3 Создание пользователя для работы (если работаете под root)

**Важно:** Docker рекомендуется запускать под обычным пользователем, а не root. Если вы подключились под root, создайте обычного пользователя:

```bash
# Создайте обычного пользователя для работы
...
```bash
# Создайте пользователя без shell доступа для запуска бота
