...
def test_csv_export():
    ...
    if not YONOTE_API_KEY or not YONOTE_CALENDAR_ID:
        print("ERROR: YONOTE_API_KEY or YONOTE_CALENDAR_ID not set in environment")
        return
    ...
    print(f"Requesting CSV export from: {csv_url}")
    ...
    try:
        ...
        # Try to parse the CSV content - handle encoding properly
        ...
        if people_like_fields:
            print(f"Found potential people fields: {people_like_fields}")
            ...
            for i, row in enumerate(reader):
                if i >= 5:  # Show first 5 rows
                    break
                print(f"Row {i+1} people data:")
                for field in people_like_fields:
                    if field in row:
                        print(f"  {field}: {row[field]}")
                print()
        else:
            print("No obvious people-related fields found")
        ...
        if date_like_fields:
            print(f"Found potential date fields: {date_like_fields}")
